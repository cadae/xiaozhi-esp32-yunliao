...
def merge_bin(lang_suffix="", chip_suffix=""):
    full_suffix = f"{chip_suffix}{lang_suffix}" if chip_suffix or lang_suffix else ""
    merge_output = f"merged_yunliao{full_suffix}.bin" if full_suffix else "merged-yunliao.bin"
...
def zip_bin(board_type, project_version, lang_suffix="", chip_suffix=""):
    if not os.path.exists("releases"):
        os.makedirs("releases")
    
    # 根据语言后缀和芯片后缀生成输入文件名和输出路径
    full_suffix = f"{chip_suffix}{lang_suffix}" if chip_suffix or lang_suffix else ""
    bin_name = f"merged_yunliao{full_suffix}.bin" if full_suffix else "merged-yunliao.bin"
    input_path = f"build/{bin_name}"
    output_path = f"releases/v{project_version}_yunliao{full_suffix}.zip"
...
def release_current():
    ...
    merge_bin(chip_suffix=chip_suffix)
    zip_bin(board_type, project_version, chip_suffix=chip_suffix)
...
def release(board_type, board_config, lang_suffix=""):
    # 配置文件路径
    ...
    for build in builds:
        ...
        output_path = f"releases/v{project_version}_{name}{chip_suffix}{lang_suffix}.zip"
        if os.path.exists(output_path):
        ...
        # Call merge-bin
        merge_bin(lang_suffix, chip_suffix)
        # Zip bin
        zip_bin(name, project_version, lang_suffix, chip_suffix)
        print("-" * 80)
    ...
    if args.board:
        ...
        for board_config, board_type in board_configs.items():
            if sys.argv[1] == 'all' or board_type == sys.argv[1]:
                release(board_type, board_config, lang_suffix)
                found = True
        if not found:
